...
model.kub <- lm(prid1.ha ~ gostsetve + I(gostsetve^2) + I(gostsetve^3), data=koruza)
# model.kub.1 <- lm(prid1.ha ~ poly(gostsetve, degree=3, raw=TRUE), data=koruza)

anova(model.kub)
anova(model.kvad, model.kub)

# izpis ocen parametrov modela in R2
...
# Interval zaupanja za optimalno gostoto setve dobimo s samovzorčenje primerov 
# (neparametrični bootstrap), 
...
(optIZ<-quantile(b$opt,c(0.025,0.975)) ) # centilni bootstrap interval zaupanja za optimum
...
# še druga možnost uporabe funkcije Boot za bootstrap optimumov:
...
str(bs.0)
head(bs.0)
plot(koruza$gostsetve, bs.0[,3], col="blue", pch=16)
points(koruza$gostsetve, bs.0[,2], col="red", pch=16)
points(koruza$gostsetve, bs.0[,1], col="green", pch=16)

#  df=4, knots=NULL
bs.1<-bs(koruza$gostsetve, df=4) #  število vozlišč=1, degree=3
str(bs.1)
head(bs.1)
plot(koruza$gostsetve, bs.1[,4], col="black", pch=16)
points(koruza$gostsetve, bs.1[,3], col="blue", pch=16)
points(koruza$gostsetve, bs.1[,2], col="red", pch=16)
points(koruza$gostsetve, bs.1[,1], col="green", pch=16)
...
anova(model.kvad, model.bs2.0) #modela imata enako število parametrov
