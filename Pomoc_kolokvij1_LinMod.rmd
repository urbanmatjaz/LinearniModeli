...
levels(data$spr) # levli faktorske spr
#data(data) 
...
ggplot(data = data) +
  geom_point(mapping = aes(x = napovedna_spr, y = odzivna_spr)) +
  xlab("napovedna_spr (enote)") + ylab("odzivna_spr (enote)") 

# Osnovni razsevni grafikon - 2. možnost
ggplot(data, aes(x=številska_napovedna_spr, y=odzivna_spr)) + 
  geom_point() + 
  xlim(c(_,_)) +
...
# Histogrami
par(mfrow=c(1,2))
hist(data$napovedna_spr1, main="", xlab="", ylab="", breaks=20)
hist(data$napovedna_spr2, main="", xlab="", ylab="")
...
boxplot(placa$wage ~ placa$jobclass, 
        xlab = "Tip zaposlitve", ylab = "Plača (1000 $)")
boxplot(placa$wage ~ placa$health, 
        xlab = "Zdravstveno stanje", ylab = "Plača (1000 $)")
boxplot(placa$age ~ placa$health, 
        xlab = "Zdravstveno stanje", ylab = "Starost (leta)")

# Osnovni boxplot
ggplot(data, aes(x=opisna_napovedna_spr, y=odzivna_spr)) + 
  geom_boxplot() + 
...
ggplot(data, aes(x=številska_napovedna_spr, y=odzivna_spr)) + 
  ...
  facet_grid(opisna_napovedna_spr1 ~ opisna_napovedna_spr2) + 
...
boxplot(odzivna_spr ~ opisna_napovedna_spr, data = data, ylab = c("odzivna_spr (enota)"), xlab = "opsina_napovedna_spr")
...
scatterplot(odzivna_spr ~ napovedna_spr, regLine = FALSE, smooth = FALSE, boxplots = 'xy',
...
outlierTest(model) # točka je reg osamelec, če je Bonferroni p-vrednost manjša od 0.05 

# Da preveriš kako se modelske napovedi razlikujejo od dejanskih točk - regresijski osamelci
data[c(cifra1, cifra2), ]
fitted(model)[c(cifra1, cifra2)]
log(data$odzivna_spr[c(cifra1, cifra2)]) # brez log, če ga ne uporabljaš!


# Vzvodne točke:  Vzvod v odvisnosti od napovedne_spr za model
plot(data$napovedna_spr, hatvalues(model), pch = 16,
...
```{r}
# Ena napovedna spr
m0 = lm(odzivna_spr ~ napovedna_spr, data = data)

# Dve napovedni spr
m1 = lm(odzivna_spr ~ napovedna_spr1 + napovedna_spr2, data = data)
...
model = lm(odzivna_spr ~ napovedna_spr1 * napovedna_spr2, data = data)

modelska_matrika = model.matrix(model.m2)
head(modelska_matrika, n = 3)

model.m2$coeff # Poglej število ocenjenih parametrov v modelu

# ... nadaljuj z anova(model)
```
...
symbox(~ odzivna_spr, xlab = "Lambda",
...
model = lm(log(odzivna_spr) ~ napovedna_spr, data = data)
...
exp(confint(model)[1,]) # Eksponiraj IZ za intercept
...
exp(confint(model)[2,])-1 # Eksponiraj IZ za beta1
summary(model)$r.squared

# Grafični prikazi za exp zvezo

## 1.1 način
plot(effect(c("napovedna_spr","lahko dodaš več napovednih"), model,
...
## 1.2 način
plot(Effect(c("napovedna_spr1", "napovedna_spr2", "napovedna_spr3"), model,
            transformation = list(link = log,inverse = exp)),
...
## 2. način
ggplot(data = data, aes(x = napovedna_spr, y = log(odzivna_spr))) +
...
ggplot(data = data, aes(x = napovedna_spr, y = odzivna_spr)) +
  geom_point() + xlab("napovedna_spr (enote)") + ylab("odzivna_spr (enote)") +
  stat_function(fun = function(napovedna_spr) exp(b[1] + b[2] * napovedna_spr)) #  Odvisnost odzivna_spr od napovedna_spr; eksponentni model
...
avPlots(m1, ylim = c(_, _)) # Vpliv vsake od napovednih spr na odzivno spr ob upoštevanju ostalih spr v modelu (= pri konstantni vrednosti ostalih spr) - naklon je enak oceni parametrov za tisto spremenljivko

# 3D grafikon za dve številski spremenljivki hkrati
# avPlot3d(mod2, coef1="Age", coef2="Ht")

# Graf parcialnih ostankov
crPlots(m1, ylim = c(_, _))# Odvisnosti odzivne spr od posamezne napovedne spr upoštevajoč drugo/-e spr v modelu → BREZ INTERAKCIJ
plot(Effect(c("spr1", "spr2", "spr3"), model), multiline=TRUE,
     ci.style="bands", main="", ylim=c(_,_)) #  Modelske napovedi za odzivno_spr v odvisnosti od spr1, spr2 in spr3 hkrati, pri povprečni vrendosti spr4 za model → Z INTERAKCIJAMI (iste grafe narišeš pred in po vključitvijo interakcij!!) → glej interakcije
...
plot(predictorEffects(m1, ~.), ylim = c(_,_), main = "") # Napovedane vrednosti in 95 % intervali zaupanja za povprečno odzivno_spr za m1; levo: glede na napovedno_spr1 pri povprečni napovedni_spr2, desno: glede na napovedno_spr2 pri povprečni napovedni_spr1.

#!!! graf parcialnih ostankov razlaga: 
plot(predictorEffect(c("napovedna_spr1"), model))
...
# Cel izpis za model
summary(m0)
names(summary(m0)) # za dodatne ali posamezne izpise

# SSmodel in SSresidual
anova(m0)

# Intervala zaupanja za parametra modela
confint(m0) # Interpretacija: Če se napovedna_spr poveča za 1 (enota), pri 95 % zaupanju pričakujemo, da se bo odzivna_spr povečala na intervalu spodnja_meja (enota) do zgornja_meja (enota). → lahko pri interpretaciji tudi množiš vse z 10 ali 100

# F-test za dva gnezdena modela: 
anova(model1, model2) #prvi je gnezden
...
graf1 = plot(Effect(c("napovedna_spr1", "napovedna_spr2"), model, partial.residuals = TRUE), 
             ci.style = "none", lattice = list(layout = c(4, 1)))
graf2 = plot(Effect(c("napovedna_spr2", "napovedna_spr1"), model.m1, partial.residuals = TRUE),
             ci.style = "none", lattice = list(layout = c(4, 1)))
...
plot(Effect(c("številska_napovedna_spr", "opisna_napovedna_spr"), model), # lahko isti graf narediš brez/z interakcijo
